...
export default function VerifyOrder({ raffleId }: Props) {
	...
	const fetchOrders = async (buyerId: string, raffleId: string) => {
		const res = await fetch(`/api/verificatuorden?buyerId=${buyerId}&raffleId=${raffleId}`, {
			cache: "no-store",
		});
	...
	const handleSearch = async () => {
		await fetchOrders(buyerId, raffleId);
		setBuyerId("");
	};
					...
					<ul className="flex flex-col gap-4 p-4 rounded-lg shadow-md  w-full">
						<div className="flex flex-col mx-auto justify-center items-center gap-2 p-4 w-full md:w-1/2">
							<p className="text-xl text-center font-bold text-pink-500 text-pretty">
								✨Si tu numero es aprobado felicidades ya estas participando en el
								sorteo, si esta pendiente debes esperar por aprobación
							</p>
							<span className="text-amber-500 text-2xl font-bold">
								{typeof orders[0]?.raffleId === "object"
							...
							return (
								...
								>
									<span className={`badge font-bold ${isPending} badge-lg mb-2`}>
										Estado:{" "}
										{order.status === "completed" ? "Aprobado" : "Pendiente"}
									</span>
